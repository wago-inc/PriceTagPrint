...
{
    /// <summary>
    /// HokaidoSankiView.xaml の相互作用ロジック
    /// </summary>
    public partial class HokaidoSankiView : MetroWindow
    {
        ...
        private void TextBox_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txt.Text))
            {
                ...
                {
                    // 変更通知が飛ばないため一旦-1をセット
                    ((HokaidoSankiViewModel)this.DataContext).SelectedHakkouTypeIndex.Value = -1;
                    ((HokaidoSankiViewModel)this.DataContext).SelectedHakkouTypeIndex.Value = 0;
                }
                else if (txt.Name == "NefudaBangouText")
                {
                    // 変更通知が飛ばないため一旦-1をセット
                    ((HokaidoSankiViewModel)this.DataContext).SelectedNefudaBangouIndex.Value = -1;
                    ((HokaidoSankiViewModel)this.DataContext).SelectedNefudaBangouIndex.Value = 0;
                }
            }
        }
        public HokaidoSankiView()
        {
            InitializeComponent();
            this.HakkouTypeText.Focus();
            // ViewModelからも初期フォーカスをセットできるように発行区分テキストを渡す。
            ((HokaidoSankiViewModel)this.DataContext).HakkouTypeTextBox = this.HakkouTypeText;
            ((HokaidoSankiViewModel)this.DataContext).JusinDatePicker = this.JusinbiDatePicker;
            ((HokaidoSankiViewModel)this.DataContext).NouhinDatePicker = this.NouhinbiDatePicker;
        }
        ...
        private void ExecuteCommand(object sender, RoutedEventArgs e)
        {
            ...
            switch (parameter)
            {
                case "Esc":
                    ...
                    break;
                case "F4":
                    ((HokaidoSankiViewModel)this.DataContext).Clear();
                    this.HakkouTypeText.Focus();
                    this.HakkouTypeText.SelectAll();
                    break;
                case "F5":
                    if (((HokaidoSankiViewModel)this.DataContext).InputCheck())
                    {
                        ((HokaidoSankiViewModel)this.DataContext).NefudaDataDisplay();
                        this.HakkouTypeText.Focus();
                        this.HakkouTypeText.SelectAll();
                    }
                    break;
                case "F10":
                    if (((HokaidoSankiViewModel)this.DataContext).PrintCheck())
                    {
                        if (MessageBox.Show("値札の発行を行いますか？", "値札発行確認", MessageBoxButton.OKCancel, MessageBoxImage.Question) == MessageBoxResult.OK)
                        {
                            ((HokaidoSankiViewModel)this.DataContext).ExecPrint(true);
                            this.HakkouTypeText.Focus();
                            this.HakkouTypeText.SelectAll();
                        }                            
                    }
                ...
                case "F12":
                    if (((HokaidoSankiViewModel)this.DataContext).PrintCheck())
                    {
                        if (MessageBox.Show("値札の発行を行いますか？", "値札発行確認", MessageBoxButton.OKCancel, MessageBoxImage.Question) == MessageBoxResult.OK)
                        {
                            ((HokaidoSankiViewModel)this.DataContext).ExecPrint(false);
                            this.HakkouTypeText.Focus();
                            this.HakkouTypeText.SelectAll();
                        }                            
                    }
            ...
            if (e.Key == Key.Return)
            {
                if (!string.IsNullOrEmpty(this.HakkouTypeText.Text))
                {
                    if (((HokaidoSankiViewModel)this.DataContext).InputCheck())
                    {
                        ...
                        ((HokaidoSankiViewModel)this.DataContext).SttHincd.Value = this.SttHincdText.Text;
                        ((HokaidoSankiViewModel)this.DataContext).EndHincd.Value = this.EndHincdText.Text;
                        ((HokaidoSankiViewModel)this.DataContext).NefudaDataDisplay();
                        this.HakkouTypeText.Focus();
                        this.HakkouTypeText.SelectAll();
                    }
