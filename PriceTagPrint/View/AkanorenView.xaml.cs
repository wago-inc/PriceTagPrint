...
{
    /// <summary>
    /// AkanorenView.xaml の相互作用ロジック
    /// </summary>
    public partial class AkanorenView : MetroWindow
    {
        ...
        private void TextBox_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txt.Text))
            {
                ...
                {
                    // 変更通知が飛ばないため一旦-1をセット
                    ((AkanorenViewModel)this.DataContext).SelectedHakkouTypeIndex.Value = -1;
                    ((AkanorenViewModel)this.DataContext).SelectedHakkouTypeIndex.Value = 0;
                }
                else if (txt.Name == "NefudaBangouText")
                {
                    // 変更通知が飛ばないため一旦-1をセット
                    ((AkanorenViewModel)this.DataContext).SelectedNefudaBangouIndex.Value = -1;
                    ((AkanorenViewModel)this.DataContext).SelectedNefudaBangouIndex.Value = 0;
                }
            }
        }
        ...
        public AkanorenView()
        {
            ...
            ((AkanorenViewModel)this.DataContext).HakkouTypeTextBox = this.HakkouTypeText;
            ((AkanorenViewModel)this.DataContext).JusinDatePicker = this.JusinbiDatePicker;
            ((AkanorenViewModel)this.DataContext).NouhinDatePicker = this.NouhinbiDatePicker;
        }
        ...
        private void ExecuteCommand(object sender, RoutedEventArgs e)
        {
            ...
            switch (parameter)
            {
                case "Esc":
                    ...
                    break;
                case "F4":
                    ((AkanorenViewModel)this.DataContext).Clear();
                    this.HakkouTypeText.Focus();
                    this.HakkouTypeText.SelectAll();
                    break;
                case "F5":
                    if (((AkanorenViewModel)this.DataContext).InputCheck())
                    {
                        ((AkanorenViewModel)this.DataContext).NefudaDataDisplay();
                        this.HakkouTypeText.Focus();
                        this.HakkouTypeText.SelectAll();
                    }
                    break;
                case "F10":
                    if (((AkanorenViewModel)this.DataContext).PrintCheck())
                    {
                        if (MessageBox.Show("値札の発行を行いますか？", "値札発行確認", MessageBoxButton.OKCancel, MessageBoxImage.Question) == MessageBoxResult.OK)
                        {
                            ((AkanorenViewModel)this.DataContext).ExecPrint(true);
                            this.HakkouTypeText.Focus();
                            this.HakkouTypeText.SelectAll();
                        }
                    }
                ...
                case "F12":
                    if (((AkanorenViewModel)this.DataContext).PrintCheck())
                    {
                        if (MessageBox.Show("値札の発行を行いますか？", "値札発行確認", MessageBoxButton.OKCancel, MessageBoxImage.Question) == MessageBoxResult.OK)
                        {
                            ((AkanorenViewModel)this.DataContext).ExecPrint(false);
                            this.HakkouTypeText.Focus();
                            this.HakkouTypeText.SelectAll();
                        }
                    }
            ...
            if (e.Key == Key.Return)
            {
                if (!string.IsNullOrEmpty(this.HakkouTypeText.Text))
                {
                    if (((AkanorenViewModel)this.DataContext).InputCheck())
                    {
                        ...
                        ((AkanorenViewModel)this.DataContext).SttHincd.Value = this.SttHincdText.Text;
                        ((AkanorenViewModel)this.DataContext).EndHincd.Value = this.EndHincdText.Text;
                        ((AkanorenViewModel)this.DataContext).NefudaDataDisplay();
                        this.HakkouTypeText.Focus();
                        this.HakkouTypeText.SelectAll();
                    }
