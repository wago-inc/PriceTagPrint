...
{
    /// <summary>
    /// SunnyMartView.xaml の相互作用ロジック
    /// </summary>
    public partial class SunnyMartView : MetroWindow
    {
        ...
        private void TextBox_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txt.Text))
            {
                ...
                {
                    // 変更通知が飛ばないため一旦-1をセット
                    ((SunnyMartViewModel)this.DataContext).SelectedHakkouTypeIndex.Value = -1;
                    ((SunnyMartViewModel)this.DataContext).SelectedHakkouTypeIndex.Value = 0;
                }
                else if (txt.Name == "BunruiCodeText")
                {
                    // 変更通知が飛ばないため一旦-1をセット
                    ((SunnyMartViewModel)this.DataContext).SelectedBunruiCodeIndex.Value = -1;
                    ((SunnyMartViewModel)this.DataContext).SelectedBunruiCodeIndex.Value = 0;
                }
                else if (txt.Name == "NefudaBangouText")
                {
                    // 変更通知が飛ばないため一旦-1をセット
                    ((SunnyMartViewModel)this.DataContext).SelectedNefudaBangouIndex.Value = -1;
                    ((SunnyMartViewModel)this.DataContext).SelectedNefudaBangouIndex.Value = 0;
                }
            }
        }

        public SunnyMartView()
        {
            InitializeComponent();
            this.HakkouTypeText.Focus();
            ((SunnyMartViewModel)this.DataContext).HakkouTypeTextBox = this.HakkouTypeText;
            ((SunnyMartViewModel)this.DataContext).JusinDatePicker = this.JusinbiDatePicker;
            ((SunnyMartViewModel)this.DataContext).NouhinDatePicker = this.NouhinbiDatePicker;
        }
        ...
        private void ExecuteCommand(object sender, RoutedEventArgs e)
        {
            ...
            switch (parameter)
            {
                case "Esc":
                    ...
                    break;
                case "F4":
                    ((SunnyMartViewModel)this.DataContext).Clear();
                    this.HakkouTypeText.Focus();
                    this.HakkouTypeText.SelectAll();
                    break;
                case "F5":
                    if (((SunnyMartViewModel)this.DataContext).InputCheck())
                    {
                        ((SunnyMartViewModel)this.DataContext).NefudaDataDisplay();
                        this.HakkouTypeText.Focus();
                        this.HakkouTypeText.SelectAll();
                    }
                    break;
                case "F10":
                    if (((SunnyMartViewModel)this.DataContext).PrintCheck())
                    {
                        if (MessageBox.Show("値札の発行を行いますか？", "値札発行確認", MessageBoxButton.OKCancel, MessageBoxImage.Question) == MessageBoxResult.OK)
                        {
                            ((SunnyMartViewModel)this.DataContext).ExecPrint(true);
                            this.HakkouTypeText.Focus();
                            this.HakkouTypeText.SelectAll();
                        }
                    }
                ...
                case "F12":
                    if (((SunnyMartViewModel)this.DataContext).PrintCheck())
                    {
                        if (MessageBox.Show("値札の発行を行いますか？", "値札発行確認", MessageBoxButton.OKCancel, MessageBoxImage.Question) == MessageBoxResult.OK)
                        {
                            ((SunnyMartViewModel)this.DataContext).ExecPrint(false);
                            this.HakkouTypeText.Focus();
                            this.HakkouTypeText.SelectAll();
                        }
                    }
            ...
            if (e.Key == Key.Return)
            {
                if (!string.IsNullOrEmpty(this.HakkouTypeText.Text))
                {
                    if (((SunnyMartViewModel)this.DataContext).InputCheck())
                    {
                        ...
                        ((SunnyMartViewModel)this.DataContext).SttHincd.Value = this.SttHincdText.Text;
                        ((SunnyMartViewModel)this.DataContext).EndHincd.Value = this.EndHincdText.Text;
                        ((SunnyMartViewModel)this.DataContext).NefudaDataDisplay();
                        this.HakkouTypeText.Focus();
                        this.HakkouTypeText.SelectAll();
                    }
