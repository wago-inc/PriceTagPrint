    ...
    public partial class ManekiView : Window
    {
        ...
        private void TextBox_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txt.Text))
            {
                ...
                {
                    // 変更通知が飛ばないため一旦-1をセット
                    ((ManekiViewModel)this.DataContext).SelectedHakkouTypeIndex.Value = -1;
                    ((ManekiViewModel)this.DataContext).SelectedHakkouTypeIndex.Value = 0;
                }
                else if (txt.Name == "NefudaBangouText")
                {
                    // 変更通知が飛ばないため一旦-1をセット
                    ((ManekiViewModel)this.DataContext).SelectedNefudaBangouIndex.Value = -1;
                    ((ManekiViewModel)this.DataContext).SelectedNefudaBangouIndex.Value = 0;
                }
            }
        }
        ...
        public ManekiView()
        {
            ...
            ((ManekiViewModel)this.DataContext).HakkouTypeTextBox = this.HakkouTypeText;
        }
        ...
        private void ExecuteCommand(object sender, RoutedEventArgs e)
        {
            ...
            switch (parameter)
            {
                case "Esc":
                    ...
                    break;
                case "F4":
                    ((ManekiViewModel)this.DataContext).Clear();
                    this.HakkouTypeText.Focus();
                    this.HakkouTypeText.SelectAll();
                    break;
                case "F5":
                    if (((ManekiViewModel)this.DataContext).InputCheck())
                    {
                        ((ManekiViewModel)this.DataContext).NefudaDataDisplay();
                        this.HakkouTypeText.Focus();
                        this.HakkouTypeText.SelectAll();
                    }
                    break;
        ...
        private void TextBox_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Return)
            {
                if (!string.IsNullOrEmpty(this.HakkouTypeText.Text))
                {
                    ...
                    {
                        int nefudaBangou;
                        ((ManekiViewModel)this.DataContext).NefudaBangouText.Value = int.TryParse(this.NefudaBangouText.Text, out nefudaBangou) ? nefudaBangou : 0;
                        ((ManekiViewModel)this.DataContext).NefudaDataDisplay();
                        this.HakkouTypeText.Focus();
                        this.HakkouTypeText.SelectAll();
                    }
