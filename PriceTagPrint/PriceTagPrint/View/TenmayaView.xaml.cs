    ...
    public partial class TenmayaView : Window
    {
        ...
        private void TextBox_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txt.Text))
            {
                ...
                {
                    // 変更通知が飛ばないため一旦-1をセット
                    ((TenmayaViewModel)this.DataContext).SelectedNefudaBangouIndex.Value = -1;
                    ((TenmayaViewModel)this.DataContext).SelectedNefudaBangouIndex.Value = 0;
                }
            }
        }
        public TenmayaView()
        {
            ...
            ((TenmayaViewModel)this.DataContext).HakkouTypeTextBox = this.HakkouTypeText;
            ((TenmayaViewModel)this.DataContext).JusinDatePicker = this.JusinbiDatePicker;
            ((TenmayaViewModel)this.DataContext).NouhinDatePicker = this.NouhinbiDatePicker;
        }
        ...
        private void ExecuteCommand(object sender, RoutedEventArgs e)
        {
            ...
            switch (parameter)
            {
                case "Esc":
                    ...
                    break;
                case "F4":
                    ((TenmayaViewModel)this.DataContext).Clear();
                    this.HakkouTypeText.Focus();
                    this.HakkouTypeText.SelectAll();
                    break;
                case "F5":
                    if (((TenmayaViewModel)this.DataContext).InputCheck())
                    {
                        ((TenmayaViewModel)this.DataContext).NefudaDataDisplay();
                        this.HakkouTypeText.Focus();
                        this.HakkouTypeText.SelectAll();
                    }
                    break;
            ...
            if (e.Key == Key.Return)
            {
                if (!string.IsNullOrEmpty(this.HakkouTypeText.Text))
                {
                    if (((TenmayaViewModel)this.DataContext).InputCheck())
                    {
                        ...
                        ((TenmayaViewModel)this.DataContext).SttHincd.Value = this.SttHincdText.Text;
                        ((TenmayaViewModel)this.DataContext).EndHincd.Value = this.EndHincdText.Text;
                        ((TenmayaViewModel)this.DataContext).NefudaDataDisplay();
                        this.HakkouTypeText.Focus();
                        this.HakkouTypeText.SelectAll();
                    }
